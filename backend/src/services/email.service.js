...
const emailService = {
  async sendReservaStatusUpdate({ correoInvitado, nombreInvitado, tituloAlojamiento, ubicacion, fechaIngreso, duracionDias, nuevoEstado }) {
    ...
    const mensajePrincipal = aceptada
      ? `¡Buenas noticias! El anfitrión ha <strong>aceptado</strong> tu solicitud de reserva.`
      : `Lamentablemente el anfitrión ha <strong>rechazado</strong> tu solicitud de reserva.`;
          ...
          <div style="background: #fff; padding: 28px 32px; border: 1px solid #E0EEEC; border-top: none;">
            ...
            <div style="background: #F5FFFE; border-radius: 10px; padding: 20px 24px; margin-bottom: 24px;">
              <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
                ...
                <tr><td style="padding: 6px 0; color: #4A6B68;">Ubicación</td><td style="padding: 6px 0;">${ubicacion}</td></tr>
                <tr><td style="padding: 6px 0; color: #4A6B68;">Fecha de ingreso</td><td style="padding: 6px 0;">${fecha}</td></tr>
                <tr><td style="padding: 6px 0; color: #4A6B68;">Duración</td><td style="padding: 6px 0;">${duracionDias} ${duracionDias === 1 ? 'día' : 'días'}</td></tr>
              </table>
            </div>
          </div>
          <div style="background: #F0F8F7; padding: 16px 32px; border-radius: 0 0 12px 12px; text-align: center;">
            <p style="color: #7A9E9B; font-size: 12px; margin: 0;">© NidoApp — Este correo fue enviado automáticamente.</p>
          </div>
        </div>
      `,
    });
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto;">
          ...
          <p style="color: #4A6B68; font-size: 14px;">Si no solicitaste este código, ignora este correo.</p>
        </div>
      `,
    });
  },
